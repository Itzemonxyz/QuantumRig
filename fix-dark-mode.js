...
files.forEach(file => {
  if (fs.existsSync(file)) {
    ...
    content = content.replace(/border-slate-200(?!\/)(?! dark:border-slate)/g, 'border-slate-200 dark:border-slate-800');
    // Replace border-slate-100 without dark:border-slate-xxx
    content = content.replace(/border-slate-100(?!\/)(?! dark:border-slate)/g, 'border-slate-100 dark:border-slate-800/60');
