...
export default function Checkout() {
  ...
  const [appliedCoupon] = useState<Coupon | null>(
  ...
  const [paymentMethod, setPaymentMethod] = useState<
    "Cash on Delivery" | "Manual Payment" | "Credit Card"
  >("Cash on Delivery");
  ...
  if (appliedCoupon) {
    if (appliedCoupon.applicableProductIds?.length > 0) {
      ...
      discountAmount =
        applicableTotal * (appliedCoupon.discountPercentage / 100);
    } else {
  ...
  const submitOrder = async (stripeTxId?: string) => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        transactionId:
          stripeTxId ||
          (paymentMethod === "Manual Payment" ? transactionId : undefined),
      };
      ...
      if (stripeTxId && res.id && !res.id.startsWith("mock_")) {
        await api.put(
          `/orders/${res.id}/pay`,
          { paymentIntentId: stripeTxId },
          token,
        );
      }

      const generatedOrderId =
        res.id || Math.floor(100000 + Math.random() * 900000).toString();
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4">
          <Link
            to="/profile"
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md shadow-indigo-600/20 text-sm"
          >
            Track Order Status
          </Link>
          <Link
            to="/"
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 px-6 py-3 rounded-xl font-bold transition-all text-sm"
          >
            Back to Home
          </Link>
        </div>
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
          No active items in checkout
        </h2>
        <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-md mx-auto">
          Your cart is empty. Let's add some custom PC parts before attempting
          checkout.
        </p>
        <Link
          to="/"
          className="inline-flex items-center justify-center px-6 py-3 text-base font-medium rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition-all duration-300"
        >
          Browse Catalog
        </Link>
      </div>
  ...
  return (
    <div
      ...
      id="checkout-view-root"
    >
      <div className="flex items-center mb-8">
        <Link
          to="/cart"
          className="hover:bg-slate-200 dark:bg-slate-700 p-2 rounded-full transition-colors mr-2 flex items-center justify-center"
        >
          <ArrowLeft className="w-5 h-5 text-slate-600 dark:text-slate-400" />
        </Link>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight ml-2">
      ...
      {!token ? (
        <div className="max-w-lg mx-auto text-center py-12 px-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mb-8 max-w-md mx-auto leading-relaxed">
            Please log in or register an account to enter shipping
            configurations and place verified hardware orders.
          </p>
          <Link
            to="/login"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 md:p-8 shadow-sm space-y-6">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-1">
                  Shipping & Delivery Details
                </h2>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Your order will be packaged securely and shipped to the
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                  ...
                  <span className="text-slate-400 font-normal shadow-none">
                    (Optional)
                  </span>
                </label>
                <textarea
              ...
              <div className="border-t border-slate-100 dark:border-slate-800/60 pt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <label
                    className={`block p-4 border rounded-xl cursor-pointer transition-all duration-200 ${paymentMethod === "Cash on Delivery" ? "border-indigo-600 bg-indigo-50/40 shadow-sm" : "border-slate-200 dark:border-slate-800 hover:border-indigo-200 bg-white dark:bg-slate-900"}`}
                  >
                  ...
                  <label
                    className={`block p-4 border rounded-xl cursor-pointer transition-all duration-200 ${paymentMethod === "Manual Payment" ? "border-indigo-600 bg-indigo-50/40 shadow-sm" : "border-slate-200 dark:border-slate-800 hover:border-indigo-200 bg-white dark:bg-slate-900"}`}
                  >
                    <div className="flex items-center space-x-3">
                      <input
                        type="radio"
                      ...
                      <div>
                        ...
                        <span className="block text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                          bKash, Nagad, Rocket Instant Gateways
                        </span>
                      </div>
                    </div>
                  </label>
                  <label
                    className={`block p-4 border rounded-xl cursor-pointer transition-all duration-200 ${paymentMethod === "Credit Card" ? "border-indigo-600 bg-indigo-50/40 shadow-sm" : "border-slate-200 dark:border-slate-800 hover:border-indigo-200 bg-white dark:bg-slate-900"}`}
                  >
                    <div className="flex items-center space-x-3">
                      <input
                        type="radio"
                ...
                <div className="bg-amber-50 border border-amber-200/80 p-5 rounded-2xl space-y-4 animate-in fade-in duration-200">
                  <div className="text-sm text-amber-900">
                    <div className="font-bold mb-3 flex flex-wrap items-center gap-2">
                      Please send total amount to our bKash/Nagad Personal
                      Number:
                      <div className="inline-flex items-center gap-2 bg-white dark:bg-slate-900 px-3 py-1.5 rounded-lg border border-amber-200 shadow-sm align-middle">
                        <strong className="text-indigo-600 tracking-wider font-mono text-sm">
                          01759231313
                        </strong>
                        <button
                          type="button"
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed">
                      Enter your Transaction ID below to verify your payment.
                      Your credentials will be processed & confirmed within 12
                      hours.
                    </p>
                  </div>
                  <div>
                ...
                >
                  {loading
                    ...
                    : paymentMethod === "Credit Card"
                      ...
                      : `Complete Order & Place Receipt (৳${Number(total || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm sticky top-24 space-y-6">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800/60 pb-3">
                Checkout Invoice
              </h2>
                  ...
                  {cart.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-xs font-medium flex items-center">
                          Qty: {item.quantity} ×{" "}
                          <TakaIcon className="w-3.5 h-3.5 mx-[1px]" />{" "}
                          {Number(
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200 text-xs flex items-center">
                        <TakaIcon className="w-3.5 h-3.5 mr-[1px]" />{" "}
                        {Number(
                          (item.product.discountPrice || item.product.price) *
                            item.quantity,
                        ).toLocaleString("en-BD", {
              ...
              <div className="space-y-3 border-t border-slate-100 dark:border-slate-800/60 pt-4 text-sm font-medium">
                <div className="flex justify-between text-slate-600 dark:text-slate-400">
                  ...
                  <span className="flex items-center">
                    <TakaIcon className="w-3.5 h-3.5 mr-[1px]" />{" "}
                    {Number(subtotal || 0).toLocaleString("en-BD", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-emerald-600 font-bold">
                    ...
                    <span className="flex items-center">
                      -<TakaIcon className="w-3.5 h-3.5 mr-[1px]" />{" "}
                      {Number(discountAmount || 0).toLocaleString("en-BD", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </span>
                ...
                <div className="flex justify-between text-slate-600 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800/60 pb-3">
                  ...
                  <span className="flex items-center">
                    {shipping === 0 ? (
                      ...
                      <>
                        <TakaIcon className="w-3.5 h-3.5 mr-[1px]" />
                        {Number(shipping || 0).toLocaleString("en-BD", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </>
                ...
                <div className="pt-3 flex justify-between font-bold text-lg text-slate-900 dark:text-white">
                  ...
                  <span className="flex items-center">
                    <TakaIcon className="w-[18px] h-[18px] mr-[1px]" />{" "}
