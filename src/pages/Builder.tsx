...
import { Check, AlertTriangle, Plus, ShoppingBag, Copy, CheckCircle2, Cpu, CircuitBoard, MemoryStick, HardDrive, Monitor, Keyboard, Mouse, Headphones, Box, Video, BatteryCharging, Fan } from 'lucide-react';

const getCategoryIcon = (categoryId: string) => {
  const id = categoryId.toLowerCase();
  if (id.includes('processor') || id === 'c1') return <Cpu className="w-5 h-5" />;
  if (id.includes('motherboard') || id === 'c2') return <CircuitBoard className="w-5 h-5" />;
  if (id.includes('ram') || id.includes('memory') || id === 'c3') return <MemoryStick className="w-5 h-5" />;
  if (id.includes('storage') || id.includes('ssd') || id.includes('hdd') || id === 'c4') return <HardDrive className="w-5 h-5" />;
  if (id.includes('gpu') || id.includes('graphics') || id === 'c5') return <Video className="w-5 h-5" />;
  if (id.includes('power') || id.includes('psu') || id === 'c6') return <BatteryCharging className="w-5 h-5" />;
  if (id.includes('case') || id.includes('chassis') || id === 'c7') return <Box className="w-5 h-5" />;
  if (id.includes('cooler') || id.includes('cooling')) return <Fan className="w-5 h-5" />;
  if (id.includes('monitor') || id.includes('display') || id === 'c9') return <Monitor className="w-5 h-5" />;
  if (id.includes('keyboard') || id === 'c8') return <Keyboard className="w-5 h-5" />;
  if (id.includes('mouse') || id === 'c10') return <Mouse className="w-5 h-5" />;
  if (id.includes('headphone') || id.includes('audio')) return <Headphones className="w-5 h-5" />;
...
export default function Builder() {
  ...
  const addAllToCart = () => {
    ...
    setIsAdded(true);
    Object.values(builderCart).forEach(p => addToCart(p, 1, true));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 min-w-[250px]">
          ...
          <div className="flex gap-2 mt-4 w-full">
            <motion.button 
              layout
              whileHover={Object.values(builderCart).length > 0 ? { scale: 1.02 } : {}}
              whileTap={Object.values(builderCart).length > 0 ? { scale: 0.95 } : {}}
              animate={isAdded ? {
                scale: [1, 1.1, 1],
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="col-span-1 lg:col-span-2 space-y-4">
          {categories.filter(c => !c.name.toLowerCase().includes('laptop') && !['c11'].includes(c.id)).map(category => {
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${selected ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                       {selected ? <Check className="w-5 h-5" /> : getCategoryIcon(category.id)}
                    </div>
                    <div>
