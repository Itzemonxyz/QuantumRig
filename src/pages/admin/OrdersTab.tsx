              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  {order.status !== 'Pending' && order.status !== 'Cancelled' && (
                    <div>
                      <h4 className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider flex items-center gap-1.5 hover:text-indigo-600 transition-colors cursor-pointer group" title="Update before changing status to Shipped">
                    ...
                    {order.items.map((item, idx) => (
                      <li key={idx} className="flex justify-between border-b border-slate-200/60 pb-2 last:border-0 last:pb-0 font-sans">
                        <div className="flex-1 pr-4">
                          <span className="text-slate-700 font-semibold block">{item.quantity}x {item.title}</span>
                          ...
                          {item.warranty && (
                            <span className="text-[10px] text-slate-500 font-medium font-sans mt-0.5 block">
                              Warranty: {item.warranty}
                            </span>
                          )}
                        </div>
                        <span className="font-bold text-slate-900 self-start shrink-0">৳{Number((item.price * item.quantity) || 0).toLocaleString("en-BD", {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                      </li>
                    ))}
                  </ul>
