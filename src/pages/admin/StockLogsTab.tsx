...
import { Search, Calendar, User, ArrowUpCircle, ArrowDownCircle, RefreshCw, ClipboardList, Filter, ArrowUp, ArrowDown, ArrowUpDown } from 'lucide-react';
...
export default function StockLogsTab() {
    ...
    <div className="p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 font-mono tracking-wide uppercase mt-1">
        ...
        <button
          onClick={fetchLogs}
          disabled={loading}
          className="flex items-center gap-2 px-3.5 py-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:text-indigo-600 disabled:opacity-50 transition-all shadow-sm"
        >
 ...
 : 'text-rose-700 hover:bg-white '
      ...
      {loading && logs.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 flex flex-col items-center justify-center shadow-sm">
          ...
          <p className="text-slate-600 font-medium">Loading stock adjustment logs...</p>
        </div>
      ) : sortedLogs.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
          ...
          <h3 className="text-sm font-bold text-slate-800">No stock logs found</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                  <th 
                    className="py-4 px-5 cursor-pointer hover:bg-slate-100 transition-colors group select-none"
                    ...
                    >
                      ...
                      <td className="py-4 px-5 whitespace-nowrap">
                        <div className="flex items-center gap-2 text-slate-600 text-xs font-medium">
                          ...
                          <span className="font-mono text-slate-500">{new Date(log.createdAt).toLocaleTimeString()}</span>
                        </div>
                      </td>

                      {/* Product details */}
                      <td className="py-4 px-5">
                        <div className="font-semibold text-slate-900 max-w-xs truncate" title={log.productTitle}>
                          {log.productTitle}
                        </div>
 ...
 : 'bg-rose-50 text-rose-700 border border-rose-100'
                      ...
                      {/* New Stock */}
                      <td className="py-4 px-5 whitespace-nowrap">
                        <span className="text-xs font-bold font-mono px-2 py-1 bg-slate-100 text-slate-800 rounded-lg border border-slate-200">
                          {log.newQuantity}
                        </span>
                      </td>
                      ...
                      <td className="py-4 px-5">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[10px] text-slate-500 truncate max-w-[150px] font-mono">
                              {log.userEmail}
                            </div>
                          </div>
                        </div>
