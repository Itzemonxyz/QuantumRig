      ...
      <div className="p-6 font-sans">
        ...
        <form onSubmit={handleSave} className="space-y-6 max-w-3xl">
          <div className="grid grid-cols-1 gap-6">
            <div>
              <label className="block text-xs uppercase font-extrabold text-slate-500 dark:text-slate-400 tracking-wider mb-2">Question Title</label>
              <input 
                required 
                type="text" 
                value={form.question} 
            ...
            <div>
              <label className="block text-xs uppercase font-extrabold text-slate-500 dark:text-slate-400 tracking-wider mb-2">Detailed Answer</label>
              <textarea 
                required 
                rows={5}
                value={form.answer} 
                onChange={e => setForm({...form, answer: e.target.value})} 
                className="w-full border border-slate-200 rounded-xl p-3 focus:ring-1 focus:ring-indigo-500 font-sans text-sm text-slate-800 dark:text-slate-200 leading-relaxed" 
                placeholder="Write a warm, helpful, structured response addressing the question clearly..." 
              />
            </div>

            <div className="w-2/5">
              <label className="block text-xs uppercase font-extrabold text-slate-500 dark:text-slate-400 tracking-wider mb-2">Sort Display Order</label>
              <input 
                required 
                type="number" 
                min="1" 
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            <button 
              ...
              className="px-5 py-2.5 text-xs uppercase font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-800 rounded-xl transition-colors"
            >
              Cancel
            </button>
            <button 
  ...
  return (
    <div className="p-6 font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight uppercase flex items-center gap-2">
            <HelpCircle className="w-5 h-5 text-indigo-600" />
            Dynamic FAQs Editor
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider font-mono mt-1">
      ...
      <div className="mb-6 flex items-center bg-white dark:bg-slate-900 border border-slate-200 rounded-xl px-3 py-2.5 max-w-md">
        ...
        <input 
          ...
          className="w-full bg-transparent text-sm focus:outline-none text-slate-800 dark:text-slate-200 font-sans" 
        />
      </div>

      <div className="overflow-x-auto border border-slate-200 rounded-2xl bg-white dark:bg-slate-900 shadow-sm">
        <table className="w-full text-left text-sm whitespace-normal">
          <thead className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border-b border-slate-200">
            <tr>
              <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider w-16 text-center">Order</th>
              <th className="px-6 py-3.5 text-xs font-bold uppercase tracking-wider max-w-xs">Question</th>
            ...
            {filteredFaqs.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                  ...
                  <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">No FAQ Items Found</p>
              ...
              filteredFaqs.map(faq => (
                <tr key={faq.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400 text-xs align-top max-w-md leading-relaxed">
                    {faq.answer}
                  </td>
                  <td className="px-6 py-4 text-right align-top">
                    <div className="flex items-center justify-end gap-2">
                      <button 
                        ...
                        title="Edit FAQ"
                        className="p-2 text-slate-600 dark:text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                      >
                      ...
                      {deleteConfirmId === faq.id ? (
                        <div className="flex items-center gap-1.5 ml-2">
                          ...
                          <button 
                            ...
                            className="px-2 py-1 text-slate-600 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 rounded drop-shadow-sm font-bold text-xs"
                          >
                            Cancel
                          </button>
                        </div>
                        ...
                        <button 
                          ...
                          title="Delete FAQ"
                          className="p-2 text-slate-600 dark:text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                        >
