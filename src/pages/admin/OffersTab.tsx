...
import { Plus, Edit2, Trash2, CheckCircle, XCircle, Loader2, Upload, Link as LinkIcon, FileText, Sparkles, AlertCircle } from 'lucide-react';
  ...
  if (isEditing) {
    return (
      <div className="p-6 max-w-2xl">
        <div className="flex items-center space-x-2 text-slate-500 dark:text-slate-400 mb-2 font-mono">
        ...
        <form onSubmit={handleSave} className="space-y-6 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 rounded-xl focus:bg-white dark:bg-slate-900 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all text-sm outline-none text-slate-800 dark:text-slate-200"
            />
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-mono">Suggested internal paths: /builder, /offers, /products</p>
          </div>
          ...
          <div>
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-normal">Supports JPEG, PNG, WEBP. Horizontal aspect ratio (e.g. 1200x500px) is best. Under 3MB.</p>
              </div>
            </div>
            ...
            {currentOffer.imageUrl && (
              <div className="mt-4 p-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 rounded-xl">
          ...
          <div className="flex space-x-4 pt-4 border-t border-slate-100">
            ...
            <button 
              ...
              className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold py-2.5 px-6 rounded-xl transition-colors text-xs uppercase tracking-wider"
            >
              Cancel
            </button>
          </div>
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center space-x-2 text-slate-500 dark:text-slate-400 mb-1 font-mono">
            <Sparkles className="w-4 h-4 text-indigo-500" />
            <span className="text-xs font-bold tracking-wider uppercase">Sales Campaigns & Promotions</span>
          </div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Manage Event Banners</h2>
        </div>
        <button 
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 flex items-center gap-2 text-xs font-mono font-bold text-slate-500 dark:text-slate-400">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-xs text-slate-400 font-mono uppercase tracking-widest bg-slate-50/50">
                <th className="py-4 px-6 font-semibold">Poster</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {offers.map(offer => (
                <tr key={offer.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <span className="font-bold text-slate-900 dark:text-white block text-sm">{offer.title}</span>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex justify-center">
                      {offer.active ? (
                        ...
                        <span className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border border-slate-150 px-2.5 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex items-center justify-end space-x-1.5">
                      <button 
                        ...
                        className="p-1.5 rounded-lg border border-slate-250 bg-white dark:bg-slate-900 hover:border-indigo-300 hover:text-indigo-650 text-slate-500 dark:text-slate-400 transition-colors disabled:opacity-50 cursor-pointer"
                        title="Edit Banner"
                      >
                      ...
                      <button 
                        ...
                        disabled={deletingId === offer.id} 
                        className="p-1.5 rounded-lg border border-slate-250 bg-white dark:bg-slate-900 hover:border-rose-300 hover:text-rose-600 text-slate-400 transition-colors disabled:opacity-50 cursor-pointer"
                        title="Delete Banner"
                      >
